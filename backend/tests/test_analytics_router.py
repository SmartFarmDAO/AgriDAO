...
class TestAnalyticsRouter:
    ...
    def regular_auth_header(self) -> Dict[str, str]:
        ...
        token = make_token(regular_user.id)
        return {"Authorization": f"Bearer {token}"}
    ...
    def test_get_dashboard_metrics_success(self, client, admin_user, mock_metrics_data):
        """Test successful dashboard metrics retrieval."""
        
        with patch('app.deps.get_current_user', return_value=admin_user), \
             patch('app.routers.analytics.MetricsCollector') as mock_collector_class:
    ...
    def test_get_dashboard_metrics_with_date_range(self, client, admin_user, mock_metrics_data):
    ...
    def test_get_dashboard_metrics_admin_required(self, client, regular_user):
    ...
    def test_get_real_time_metrics_success(self, client, admin_user):
    ...
    def test_get_revenue_analytics_success(self, client, admin_user, mock_metrics_data):
    ...
    def test_get_user_analytics_success(self, client, admin_user, mock_metrics_data):
    ...
    def test_get_product_analytics_success(self, client, admin_user, mock_metrics_data):
        ...
        with patch('app.deps.get_current_user', return_value=admin_user), \
            ...
            response = client.get("/analytics/products?limit=10")
    ...
    def test_get_order_analytics_success(self, client, admin_user, mock_metrics_data):
    ...
    def test_get_farmer_analytics_success(self, client, admin_user, mock_metrics_data):
        ...
        with patch('app.deps.get_current_user', return_value=admin_user), \
            ...
            response = client.get("/analytics/farmers?limit=5")
    ...
    def test_track_analytics_event_success(self, client, admin_auth_header):
        ...
        with patch('app.routers.analytics.MetricsCollector') as mock_collector_class:
            ...
            # Combine auth and CSRF headers
            headers = {**admin_auth_header, "X-CSRF-Token": csrf_token}
    ...
    def test_export_analytics_data_csv(self, client, admin_user, mock_metrics_data):
        ...
        with patch('app.deps.get_current_user', return_value=admin_user), \
            ...
            response = client.get("/analytics/export?format=csv&metric_type=revenue")
            
            assert response.status_code == 200
            assert response.headers["content-type"] == "text/csv; charset=utf-8"
    
    def test_export_analytics_data_json(self, client, admin_user, mock_metrics_data):
        ...
        with patch('app.deps.get_current_user', return_value=admin_user), \
            ...
            response = client.get("/analytics/export?format=json&metric_type=comprehensive")
            
            assert response.status_code == 200
            assert response.headers["content-type"] == "application/json; charset=utf-8"
    
    def test_clear_analytics_cache_success(self, client, admin_user):
        """Test cache clearing functionality."""
        
        with patch('app.deps.get_current_user', return_value=admin_user):
            response = client.delete("/analytics/cache?pattern=metrics:*")
    ...
    def test_analytics_error_handling(self, client, admin_user):
    ...
    def test_date_range_validation(self, client, admin_user, mock_metrics_data):
        ...
        with patch('app.deps.get_current_user', return_value=admin_user), \
            ...
            response = client.get("/analytics/dashboard?start_date=invalid-date")
            
            # Should handle gracefully or return validation error
            # The exact behavior depends on FastAPI's date parsing
            assert response.status_code in [200, 422]  # Either works or validation error
    
    def test_granularity_parameter(self, client, admin_user, mock_metrics_data):
        ...
        with patch('app.deps.get_current_user', return_value=admin_user), \
            ...
            response = client.get("/analytics/revenue?granularity=day")
            assert response.status_code == 200
            
            # Test invalid granularity
            response = client.get("/analytics/revenue?granularity=invalid")
